...
- Rétropropagation et composants architecturaux
...
- L’apprentissage auto-supervisé et au-delà
...
Plus tard, en 1948, la cybernétique a été proposée par Norbert Wiener. Il se base sur l'idée qu'en ayant des systèmes avec des capteurs et des actionneurs, il y a une boucle de rétroaction et un système d'auto-régulation. Les règles du mécanisme de rétroaction d'une voiture sont toutes issues de ce travail.
...
-->
## [Histoire de la reconnaissance de motifs et de l'introduction de la descente en gradient](https://www.youtube.com/watch?v=0bMe_vCZo30&t=1461s)
...
Ce qui précède constitue la base conceptuelle de la reconnaissance de motifs avant le développement de l'apprentissage profond. 
Le modèle standard de reconnaissance de motifs est constitué d'un extracteur de caractéristiques et d'un classifieur pouvant être entraîné. 
L'entrée va dans l'extracteur de caractéristiques, qui en extrait les caractéristiques utiles et pertinentes telles que la détection d'un œil lorsque le but est de reconnaître un visage. Ensuite, le vecteur de caractéristiques est envoyé au classifieur pouvant être entraîné pour calculer la somme pondérée et la comparer avec le seuil. Dans ce cas, un classifieur pouvant être entraîné peut-être un perceptron ou un simple réseau neuronal. Le problème est que l'extracteur de caractéristiques était conçu à la main. D'après ce que l'on peut trouver dans la littérature de l'époque; beaucoup d'efforts ont été fournis sur l'extracteur de caractéristiques et peu sur le classifieur entraînable.

Après l'émergence et le développement de l'apprentissage profond, le processus en deux étapes est passé à des enchaînements de modules. Chaque module a des paramètres réglables et une non-linéarité. Ensuite, il faut les empiler pour former plusieurs couches. On appelle cela : "l'apprentissage profond" (Deep learning). La raison pour laquelle on utilise la non-linéarité plutôt que la linéarité est que deux couches linéaires pourraient-être ramenée à une seule couche linéaire car la composition de deux couches linéaires est linéaire.

L'architecture multicouche la plus simple avec des paramètres réglables et la non-linéarité pourrait être la suivante : l'entrée est représentée par un vecteur tel qu'une image ou un son. Cette entrée est multipliée par la matrice de poids dont les coefficients sont des paramètres réglables. Ensuite, chaque composante du vecteur de résultat est passée dans une fonction non linéaire telle que ReLU (Rectified Linear Unit). En répétant ce processus, cela devient un réseau neuronal de base. La raison pour laquelle on appelle cela un réseau de neurones est que cette architecture calcule une somme pondérée des composantes en entrée par les lignes correspondantes d'une matrice.

Pour en revenir à l'apprentissage supervisé, nous comparons le résultat obtenu avec le résultat désiré (une cible) puis nous optimisons la fonction objectif qui nous donne la perte/erreur; on la calcule comme étant la distance/pénalité/divergence entre le résultat et la cible. Ensuite, nous faisons la moyenne de cette fonction de coût sur le jeu d’entraînement. C'est l'objectif que nous voulons minimiser. En d'autres termes, nous voulons trouver la valeur des paramètres qui minimisent cette moyenne.

La méthode pour les trouver est de calculer le gradient. Par exemple, si nous sommes perdus dans une montagne par une nuit brumeuse et que nous voulons aller au village dans la vallée. Un moyen pourrait être de tourner sur soi-même et de sentir dans quelle direction se trouve la pente la plus abrupte pour descendre, puis de faire un petit pas vers le bas. La direction est une pente (négative). En supposant que la vallée est convexe, nous pourrions atteindre la vallée.

La manière la plus efficace est appelée "descente stochastique du gradient" (SGD en anglais pour Stochastic Gradient Descent). Comme nous voulons minimiser la perte moyenne sur le jeu d’entraînement, nous prenons un échantillon ou un petit groupe d'échantillons et nous calculons l'erreur, puis nous utilisons la descente du gradient. Ensuite, nous prenons un nouvel échantillon et obtenons une nouvelle valeur pour l'erreur, puis nous obtenons le gradient qui est une direction différente normalement. Deux des principales raisons d'utiliser la SGD sont qu'elle permet à un modèle de converger rapidement de manière empirique si l'ensemble d’entraînement est très grand, et qu'elle permet une meilleure généralisation ce qui signifie obtenir des performances similaires sur différents jeux de données.
...
-->
### [Calculer les gradients par rétropropagation (Backpropagation)](https://www.youtube.com/watch?v=0bMe_vCZo30&t=2336s)
...
Le calcul des gradients par rétropropagation est une application pratique de la règle de la dérivation en chaîne. L'équation de rétropropagation pour les gradients d'entrée est la suivante :
...
Il est important de réfléchir à la manière d'interpréter les entrées. Par exemple, une image de $256$\times$256$ (avec 3 canaux R,V,B) nécessiterait une matrice d'environ 200 000 valeurs. Il s'agirait d'énormes matrices que les couches du réseau neuronal devraient manipuler. Il serait peu pratique d'utiliser de telles matrices. Par conséquent, il est important de faire des hypothèses sur la structure de la matrice.
